...
test.describe('Product Listing Page', () => {
  ...
  test('should filter by in-stock products only', async ({ page }) => {
    ...
    // Verify no "Out of Stock" badges are visible (web-first wait for the
    // filtered DOM to settle)
  ...
  test('should show correct product count', async ({ page }) => {
    ...
    // Get the count from the text. The visibility assertion above guarantees
    // the "Showing X of Y" text is present, so the digits are deterministic.
    const text = String(await resultsInfo.textContent());
    const numbers = Array.from(text.matchAll(/\d+/g), (m) => m[0]);
    expect(numbers).toHaveLength(2);

    const [showingText, totalText] = numbers;
    const showing = parseInt(showingText);
