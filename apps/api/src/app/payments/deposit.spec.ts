...
import { depositAmountCents, depositError } from './deposit';

const noDeposit = { depositType: null, depositValue: null, priceCents: 10_000 };
const percent = (depositValue: number, priceCents = 10_000) => ({
  depositType: DepositType.PERCENT,
  depositValue,
  priceCents,
});
const fixed = (depositValue: number, priceCents = 10_000) => ({
...
describe('depositAmountCents', () => {
  ...
  // niepełna para nie powinna nigdy trafić do bazy (CHECK + walidacja), ale gdyby
  // helper dostał ją z niezwalidowanego źródła, milczące 0 byłoby groźniejsze niż null
