...
@Component({
  ...
  imports: [AdminToolbar, AdminPagination, AdminStatusBadge, ConfirmDialog],
  template: `
    <app-admin-toolbar
      class="mt-6 block"
      [q]="list.params().q"
    ...
    @if (list.loading()) {
      ...
      >
        @if (list.params().page) {
          ...
          <p class="font-medium">Brak firm dla podanych filtrów.</p>
          <p class="mt-1 text-sm text-stone-500">
            Zmień frazę wyszukiwania lub filtr statusu.
          </p>
        } @else {
          <p class="font-medium">Nie ma jeszcze żadnych firm.</p>
        }
      </div>
    } @else {
      <div
